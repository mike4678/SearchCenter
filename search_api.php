...
switch($type)
{
	case 'mac':
		...
		{
			case 0:
			echo json_encode(array('status'=>'-1','message'=>'查询无果','Data'=>$querydata));
			exit;
			
			case 1:
			echo json_encode(array('status'=>'0','message'=>'Success','Data'=>array('mac'=>$searchdata,'mac_q'=>$querydata,'regcompy'=>$row['Organization']))); 
			exit;
	...
	case 'process':
		...
		switch($count)
		{
			...
			case 1:
				echo json_encode(array('status'=>'2','message'=>'Success','Data'=>array('p_name'=>$searchdata,'p_path'=>$row['path'],'regcompy'=>$row['compy'],'p_level'=>$row['safevalue'],'p_other'=>$row['other']))); 
				exit;
	...
	case 'AllowSearch':
		$row = $dou -> Info('AllowSearch');
		if($row == NULL )  
		{
			echo json_encode(array('status'=>'-1','message'=>'Failed'));
		} else {
			echo json_encode(array('status'=>'0','message'=>'Success','Data'=>$row));
		}
		break;
}
?>
